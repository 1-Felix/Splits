...
- [x] 8.1 Write the failing tests for the three missed tempos (`2026-01-09`, `2026-02-13`, `2026-02-27`): a prescribed single block is found by sliding a window of exactly the prescribed size; the best window's mean pace falls in the prescribed band; the document reads `block`, boundaries from the window. These need stream fixtures — build them from the local archive's real streams (it HAS streams; it lacks only laps). *(Truth tests over the six real runs' archive streams, archive-guarded.)*
- [x] 8.2 Write the failing tests for the three fragmented tempos (`2026-01-23`, `2026-03-13`, `2026-04-03`): the same window rule absorbs the fragments with no second mechanism.
- [x] 8.3 Write the SAFETY test before implementing: a run that plodded the prescribed window out of band (synthetic 7:00/km) refuses the block and falls back to inference — step 3 of D1a is the entire honesty property.
- [x] 8.4 Implement POINT with the within-window variance guard in place and its threshold deliberately conservative (design Open Question 1 — no archive case can set it honestly; a flat window confirms, a hard/easy rep pattern must not).
- [x] 8.5 `2026-01-23`'s 304 s mid-block gap: the merged block is HEDGED, never asserted — the case D8 split the two changes over. Write the test on the real stream fixture. *(MEASURED CORRECTION, recorded in design D1a: within-window pace cannot single out 2026-01-23 — hedging follows evidence of interruption (deep stop ≥60 s below 0.8×lo, or a bout-fragmented window). Only 2026-01-09 asserts of the six; 2026-02-13's ~98 s break and 2026-02-27's 92 s standstill hedge too, which the design's trace never examined.)*
- [x] 8.6 POINT never consults the calibration floor (closes handoff P2.3 — `2026-01-09`'s band sits below the floor and must be found anyway). *(POINT confirms with `work_floor` well above the band in the synthetic test, and at production floor 2.7 in the truth tests.)*
- [x] 8.7 Mutation-prove: breaking the band check (always confirm) turns the suite red via 8.3; breaking the variance guard (never guard) turns the suite red via its synthetic case. *(Band check deleted → the plod test fails; variance disabled → the substituted-session test fails — after tuning its window IN band (measured mps 3.086, cv 0.47) so variance is the sole refuser; reverted green.)*
...
- [x] 12.1 Merge to `main`, `gh run watch <id> --exit-status`, then on the NUC: `docker top splits` FIRST (orphan check), `docker compose pull splits && docker compose up -d splits`. Never wrap `ssh … docker …` in a client-side `timeout`. *(Run 30528035684 success; no orphans; pulled and restarted.)*
- [x] 12.2 Run `--backfill-workouts` once, out of band (confirm with Felix first — ~89 authenticated calls against his account). Expect ~85 banked, 4 gone. *(85 banked, 4 deleted (404) — the exploration's exact attrition.)*
- [x] 12.3 `curl -s -X POST http://localhost:5732/api/sync` — the recompute rescores all ~170 documents at v6. *(All 170 documents at v6.)*
- [x] 12.4 Re-run the sweep and diff against `baseline-before.md`. Expected movement is EXACTLY the named set and nothing else: the 3 missed tempos → `block`; the 3 fragmented tempos → one `block` each (hedged — interrupted executions, see D1a correction; of the missed three only `2026-01-09` asserts); `2025-12-24`, `2026-04-29`, `2026-04-01` → `steady`; `2026-07-29` → `4×20 s` and `2025-12-26` → `4×30 s` (both asserting again); `2026-06-05` → its prescribed tempo (warm-up demoted); `2026-01-16` → one rep; `2025-12-05` → `found: 2`; `2026-02-06` → `6×90 s`; `2025-11-21` → `8×90 s`. The 12 prescribed-count-matching sets must be untouched. Anything else moving is a defect. *(22 documents moved, every one an intended consequence — full audit in notes.md. Bonus: the half-marathon race corrected from a junk `6 min block` @0.52 to `139 min block` @0.9 via POINT on its race workout. The stale-edited 2025-10-17 and all 12 correct sets untouched.)*
- [x] 12.5 `--verify-archive` exits 0; `docker top splits` clean. *(exit 0; `workouts: 85/89 referenced definitions banked, 4 unbanked`; no orphans.)*
- [x] 12.6 Load `/run/:id` for `2026-07-29` (now `4×20 s`, asserting) and `2026-01-16` (one rep) and confirm the pages match the documents. *(Live: `4×20 s`, found 4/4, asserting, guidedBy carried, 4 rep rows on the page, no hedge; 2026-01-16 reads `5 min block` asserting.)*
- [x] 12.7 Update the interval-lens memory + handoff: N3, N4, N5, P2.7b, P3.1 closed; P2.3's windowed baseline formally retired; the archive-side sweep baseline updated. *(Handoff banner added; notes.md audit; memory updated.)*
