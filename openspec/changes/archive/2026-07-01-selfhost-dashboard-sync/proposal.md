...
- **Credentials in `.env`, data in a volume.** Garmin credentials live in a git-ignored `.env`; `docker-compose.yml` references them via `${VAR}` substitution and passes them to the container, so the compose file stays safe to commit. `garmin-data.js`, `plan-data.js`, and the token/raw caches live in a mounted `/data` volume and survive restarts and image upgrades. `plan-data.js` is seeded from a shipped default on first boot; the AI-coach workflow continues to edit it (now in the volume) — **untouched by this change**.
...
- **Config/secrets:** Garmin credentials, `ATHLETE_*`, `TZ`, and `SPLITS_DATA_DIR` become container environment variables; credentials are plaintext in a git-ignored `.env` referenced by `docker-compose.yml` (the standard self-host bargain, documented).
