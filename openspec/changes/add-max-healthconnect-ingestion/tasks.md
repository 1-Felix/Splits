...
- [ ] 1.4 **Decision gate:** on Max's Pixel (Samsung Health 7.00.0.107), record an outdoor run on the Galaxy Watch and confirm an `ExerciseSession` with HR actually lands in Health Connect. If Samsung does not write sessions on this build, stop and fall back to manual entry until it is fixed — *MOSTLY CLEARED 2026-07-16 on the S24: GATE: YES — Samsung Health 7.00.5.009 writes running sessions (+distance/speed/kcal) to HC, and backfilled 4 historic workouts. ROOT CAUSE of the earlier NO: Health Connect denies Samsung Health WRITE permissions by default (HC → App permissions → Samsung Health → enable Write). Remaining on Max's Pixel: version ≥7.00.5.009 (update if 7.00.0.107), grant the write toggles, record a WATCH run and confirm HR samples present (phone-recorded runs carry none)*
...
- [x] 5.2 Author Max's `plan-data.js` (beginner→half marathon, Felix + AI); validate against the plan schema — *DONE 2026-07-16: 40 weeks (Mon 2026-07-20 → race Sun 2027-04-25, spring Allgäu half TBD — re-anchor via `max-plan-generator.py` in this change folder when he registers), 3 sessions/wk Mon/Wed/Sat (long run Sat; v2 2026-07-16 — days changed per Felix, coach note reframed as three campaigns with the Wk-26 10K as the formal re-anchor checkpoint), run/walk→30 min (Wk 8)→5K test (Wk 16)→10K test (Wk 26)→tempo build→18 km peak→taper, peak 35.5 km/wk, goal sub-2:30 stretch/finish-healthy primary; `validatePlanText` ok:40 weeks; deployed to the NUC volume via docker exec (default plan backed up as plan-data.js.bak-default); browser-verified on a fresh 0-run instance. Rendering these 2027/beginner shapes exposed Felix-specific cockpit hardcodes — fixed one-image: dateFmt year + plan-start from block[0].mon + PB '—' fallback + 'Base build' countdown tier + derived ROAD TO title/subtitle + first-class rest kind; cockpit + slim-render suites green*
- [x] 5.3 Establish the phone→NUC network path (Tailscale or TLS reverse proxy) so the app reaches `/api/ingest` securely — *DONE 2026-07-16 (Felix chose the public traefik route): `https://splits-max.mochii.dev` via Cloudflare Tunnel → traefik; dashboard router keeps `pocketid-auth`, a second higher-priority router exempts `Path(/api/ingest)` (the app's own bearer token guards it). Verified end-to-end: tokenless POST → the app's 401, real-token empty-body POST → 422 validator response, dashboard → 302 to pocket-id. Gotcha fixed on the way: splits-max was never on the `proxy` network, so traefik defaulted to an unreachable network and requests hung — `networks: [proxy]` added. Bonus: proper TLS, the app's cleartext allowance stays unused*
...
- [x] 6.1 Background recurring sync (WorkManager); one-time permission grant flow; config screen for server URL + token — *DONE 2026-07-16: spike evolved in place into the production bridge (`BridgeConfig`/`RunReader`/`IngestClient`/`SyncEngine`/`SyncWorker` + config UI); periodic 6 h WorkManager job verified registered in JobScheduler on the S24; diagnostic GATE dump kept for 1.4*
- [x] 6.2 First-run backfill since a configured start date, incremental thereafter, idempotent by session UID — *verified on-device against a local dev server: 26 runs + 76 RHR days backfilled from 2026-05-01; the immediately-following background pass pushed 0 (pushed-UID set); every sync re-scans the full window so late-syncing watch workouts can't be skipped*
- [x] 6.3 Retry with no data loss when the ingest endpoint is unreachable — *verified on-device: unreachable server → transient failure, no UID marked, WorkManager Result.retry(); path restored → all 26 re-pushed, server still 26 banked (UID upsert)*
- [x] 6.4 Complete Google's Health Connect Developer Declaration for sideloaded data-type access (start early — schedule risk, not feasibility) — *RESOLVED BY RESEARCH + DECISION 2026-07-16: the standalone declaration form for non-Play apps was retired (folded into Play Console, deadline Jan 2025); today the Health apps declaration exists only in Play Console and on-device enforcement targets Play-published apps — a sideloaded app has nothing to file and no gate (matches empirical behavior on both devices). Felix chose sideload-only distribution; WATCH-ITEM: if Google gates sideloaded HC access or we want Play auto-updates, publish on a closed-testing track under the existing AirPipe account (applicationId already renamed Play-clean: `com.splits.bridge`)*
...
- [x] 7.3 Port 7.1/7.2 into the production bridge app + add the 5 new READ permissions to the manifest + update the Google Health Connect Developer Declaration — *reader code + all 11 manifest permissions already live in `android-bridge/` (the spike is the production app's base, group 6); payload mapping DONE 2026-07-16 (`IngestClient.runPayload` — verified end-to-end into a built garmin-data.js); Declaration resolved with 6.4 (sideload-only — nothing to file)*
