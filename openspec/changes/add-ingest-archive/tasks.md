...
- [x] 1.1 Pin the samples-dict contract: parity test that builds the samples
...
- [x] 1.2 Baseline the Garmin pipeline: run `test_activity_archive.py`,
...
- [x] 2.1 `derive_activity_id(session_uid)` in `ingest_builder.py`: first 12
      hex chars of sha256 → int (48-bit, JS-safe); unit tests: deterministic,
      distinct for distinct UIDs, below 2^53
- [x] 2.2 Collision rule: on upsert, existing row with a different embedded
...
- [x] 3.1 Open/create the db via `activity_archive.ensure_schema` in
      `SPLITS_DATA_DIR`; test: fresh dir → schema version + tables identical
      to a Garmin-created db
- [x] 3.2 Upsert one row per banked run: promoted columns
      ...
      assertions
- [x] 3.3 Streams synthesis → `detail_streams_json`: shared `t` axis from the
...
- [x] 3.4 `detail_distilled_json`: refactor the builder's existing
      ...
      for the same run
- [x] 3.5 `run_metrics` rows via `insight_metrics.best_efforts` +
...
- [x] 3.6 Write-once + idempotency: derived artifacts recomputed only when
...
- [x] 3.7 Wire the pass into the builder's main flow after the telemetry
...
- [x] 4.1 `test_ingest_e2e.mjs`: after a pushed run, assert the archive db
      exists and `/api/archive/activities`, `/activities/:id`, `/:id/streams`,
      and `/run-metrics` all serve the run
- [x] 4.2 `test_slim_render.mjs`: the slim (2-run) fixture now grows an
      ...
      renders
- [x] 4.3 Keep the no-archive shape covered: the Garmin-shaped `fullDir`
      fixture (no db) retains the 404 / hidden-chrome / "No archive on this
      instance" assertions moved off the slim fixture
- [x] 4.4 Full suite green: `test_ingest_builder.py`,
...
- [x] 6.1 Update the change's HANDOFF-style notes + `activity_archive.py`
      header comment: two producers, one schema, ingest archive is a
      disposable derived cache
- [x] 6.2 Note the follow-up candidates where they'll be found again:
