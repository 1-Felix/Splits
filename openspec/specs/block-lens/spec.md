# block-lens Specification

## Purpose
The deterministic block-summary engine: one lens document per training block (enumerated from plan snapshots, keyed by race), derived every sync from compliance verdicts, run metrics and race predictions into the `block_lens` table (schema v9), surfaced as the additive `blockLens` data contract and the "The Block" section on `/progress` — a live report card for the in-flight block that becomes each block's retrospective the moment its race date passes.

## Requirements

### Requirement: Blocks are enumerated from plan snapshots, keyed by race
The sync SHALL enumerate training blocks from `plan_snapshots`, identifying a
block by its `(race.name, race.date)` pair. A block's window SHALL span the
earliest `week.mon` seen for that race across its snapshots through
`race.date`, and the block's planned shape (weeks, phases, planned km) SHALL
come from the latest snapshot of that race. A block whose race date is on or
after sync-today is the current block; all others are complete.
...
and SHALL recompute completed blocks when `lens_version` differs from
`BLOCK_LENS_VERSION`. Changing any algorithm parameter SHALL require bumping
`BLOCK_LENS_VERSION`.
...
days` and the quality hit rate (planned Hard-load days executed). The lens
SHALL never re-score or re-classify a compliance verdict.
...
machine-readable reason, never an extrapolated value.
...
planned km remaining, the planned weekly-km silhouette through race day, and
plan-integrity flags for undetailed future weeks. Complete blocks SHALL carry
no forward-tilt section.
