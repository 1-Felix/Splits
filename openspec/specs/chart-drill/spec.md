# chart-drill Specification

## Purpose
The cross-cutting drill contract from aggregate chart points to the runs
behind them: a pinned reading's second activation drills into its evidence —
an honest contribution panel for pooled points, direct run links for
single-run points — lazily fetched and honestly degrading, keyboard-complete.
