# coach-loop Specification

## Purpose
Deterministic plan-compliance loop between the coach-owned training plan and the activity archive: fail-soft plan ingestion, append-only plan snapshots, coarse reasoned scoring, an independent `compliance` block in the data contract, a nightly coach briefing, and a `/coach` ritual with a validated write contract.
