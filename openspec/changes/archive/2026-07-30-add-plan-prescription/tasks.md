...
- [x] 1.1 Extract the live plan's distinct `val` strings (read-only on the
      symlink) into `tests/fixtures/plan_vals.json`, each mapped to its
      expected parse or `null` (refusal).
- [x] 1.2 `plan_prescription.py`: `prescription_for_day(segments) -> dict | None`
...
- [x] 1.3 `test_plan_prescription.py`: the fixture sweep (every string pins its
...
- [x] 1.4 Corpus-currency test: re-extract distinct strings from plan-data.js
      at test time; an unpinned new string fails with a message naming it.

## 2. The verdict in compliance

- [x] 2.1 Schema v14: guarded ALTER adds `plan_compliance.quality_json TEXT`;
      migration test (both columns present, second open no-op).
- [x] 2.2 `plan_compliance`: compute the verdict per design D2 when a matched
      ...
      = 2`.
- [x] 2.3 Tests with synthetic documents: complete set in band; bailed 2-of-4;
      ...
      (quality_json NULL).
- [x] 2.4 D3 annotate-only pin: two identical days, one with a parseable
      prescription — `status` and `reason` byte-identical. Mutation: make the
      verdict writer downgrade status → red.
- [x] 2.5 Rescore test: a week scored at version 1 gains verdicts when
      rescored at version 2, statuses unchanged.

## 3. Surfaces

- [x] 3.1 Briefing: the compliance section appends the verdict sentence on
      annotated days; test via the existing briefing fixtures.
- [x] 3.2 `serve.mjs`: by-id `plan` object carries `quality` (omitted when
      NULL); pin in `test_archive_api.mjs` both presence and absence.
- [x] 3.3 `run.dc.html`: plan card renders planned text + verdict when
...
- [x] 4.1 Full Python suite + all four JS suites green;
