# lens-confidence Specification

## Purpose
How an interval document's confidence is derived on the device path, what hedging means — a shape that survives only because the engine's own size floor removed the alternative must not assert — and the single assertion verdict (`asserts`), decided by the engine and carried in the document, that the page renders without threshold arithmetic of its own.
